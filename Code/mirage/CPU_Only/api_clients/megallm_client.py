...
Purpose: API-3 (gemini-2.5-flash) client with a three-tier provider chain, all
         calling the SAME model:
           1. MegaLLM gateway   (primary)
           2. LinkAPI gateway   (secondary, "geminicheap" pricing group)
           3. OpenRouter        (tertiary, google/gemini-2.5-flash — paid)

All three are OpenAI-compatible endpoints, so each tier uses the openai SDK with
a different base URL/key.
...
Retry/fallback policy:
  1. MegaLLM, single key, 2 attempts.
  2. LinkAPI, single key, 2 attempts.
  3. OpenRouter round-robin (2 keys) on google/gemini-2.5-flash.
...
def call_megallm_with_fallback(
    ...
    """
    Call MegaLLM (primary), then LinkAPI (secondary), then OpenRouter (tertiary)
    ...
    # Primary: MegaLLM gateway (single key)
    ...
    # Secondary: LinkAPI gateway (single key, geminicheap group), same model id
    if LINKAPI_API_KEY:
    ...
    # Tertiary: OpenRouter round-robin on the same model
