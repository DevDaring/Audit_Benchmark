...
MIRAGE is a discriminative-validity audit framework for LLM bias benchmarks. It combines behavioural probing across five probe slots with causal activation patching (CDVA), operationalising the discriminant-validity criterion of Campbell and Fiske (1959) and the construct-validity critique of Bean et al. (NeurIPS 2025). Eight models — four open-source (OSM, audited behaviourally and causally) and four API-served (behavioural only) — are evaluated on **596 audit seeds** drawn from BBQ, CrowS-Pairs, and StereoSet (N = 596 × 12 = **7,152 pentad prompts**). WinoBias seeds are held out for future multilingual and coreference extensions.
...
### Headline results (final 8-model run)

- **Validity gap.** On BBQ the open models pass the native surface test at **57.9%** but the full causal audit at **18.5%** — a hidden invalidity of **39.4 points** (Cohen's *h* = 0.84). CrowS-Pairs and StereoSet show gaps of 20.4 and 17.2 points.
- **What the gap is.** Answer instability under resampling (FM4, incidence 0.426 among native-passing pairs), not surface stereotyping, accounts for most of the gap.
- **Causal blind spot.** **43 of 2,384** open-model pairs (**1.8%**, Wilson 95% CI [1.3, 2.4]; 13.6% of MIRAGE-B passers) pass the behavioural audit yet fail the causal test. Only mechanism-level testing detects them.
- **Artifacts.** Behavioural results 81,056 rows; CDVA 23,840 located pairs (τ = 0.7644). Every number reproduces from `Code/mirage/results/`.
...
Bias benchmarks are used to certify LLMs before deployment in hiring, healthcare, education, and content moderation. A benchmark that **passes** a model does not guarantee the model is fair — it may only mean the benchmark **measures the wrong thing** (an artefact of item phrasing or answer noise rather than group bias).

Bean et al. (NeurIPS 2025) review 445 benchmarks and document construct-validity failures at scale. Their approach is a checklist applied by human reviewers. MIRAGE goes further: it is an **instrument** that operationalises discriminative validity with **mechanism-level causal intervention** (CDVA), and isolates the residual that passes a behavioural audit yet fails a causal one.
...
4. **Validity leaderboard** — A 3×5 matrix (benchmark × failure mode) quantifying where source benchmarks fail discriminative validity, with the per-benchmark validity gap.
5. **Behavioural→causal predictability** — A logistic model relates the cheap behavioural slot signals to the causal-audit outcome (high by construction, since the full score includes the behavioural score) and isolates the **1.8%** residual that passes behaviourally yet fails causally — the cases only mechanism-level testing detects.
...
| **Predictive validity** | Messick (1995) | Do cheap behavioural signals predict the causal-audit outcome? (Held-out WinoBias transfer is future work, not in the final run.) |
| **Interventional validity** | Pearl (2009); Meng et al. (2022) | Does the model respond correctly under `do(demographic_token := alternative)`? |
| **Counterfactual fairness** | Kusner et al. (2017) | Swaps within demographic equivalence classes |

MIRAGE defines five **failure modes (FM1–FM5)** that a valid bias instrument must detect, and maps each to a **probe law violation** (Section 5).
...
In the final run, predictive validity is measured **within the instrument**: a logistic model relates the behavioural slot signals to the causal-audit outcome and isolates the residual that passes behaviourally yet fails causally (1.8% of open-model pairs). A model trained on BBQ transfers to CrowS-Pairs and StereoSet, which shows the laws are not benchmark-specific noise. The original plan of training on MIRAGE features to predict held-out **WinoBias** coreference failure is **future work**; WinoBias seeds are downloaded and held out but were not evaluated in this run.
...
| WinoBias | 200 | Held out — future work (not evaluated in this run) |
| Dev set | 50 | reserved for τ calibration; the final run set τ to the 75th percentile of \|C\| (dev seeds untagged) |
